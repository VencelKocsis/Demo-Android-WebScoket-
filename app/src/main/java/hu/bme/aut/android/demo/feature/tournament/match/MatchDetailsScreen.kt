                ...
                LazyColumn(
                    ...
                    if (match.status == "finished") {
                        ...
                        if (match.individualMatches.isEmpty()) {
                            item {
                                Text(
                                    "Nincsenek feltöltve részletes eredmények.",
                            ...
                            items(match.individualMatches.size) { index ->
                                ...
                                Row(
                                    ...
                                    Text(
                                        "${game.homePlayerName} vs ${game.guestPlayerName}",
                        ...
                        item {
                            ...
                            Row(modifier = Modifier.fillMaxWidth()) {
                                ...
                                Column(modifier = Modifier.weight(1f)) {
                                    ...
                                    Spacer(modifier = Modifier.height(8.dp))
                                    match.participants.filter { it.teamSide == "HOME" }
                                        .forEach { p ->
                                            ParticipantRow(
                                                name = p.playerName,
                                                status = p.status,
                                                ...
                                                onToggle = {
                                                    viewModel.onEvent(
                                                        MatchDetailsEvent.OnToggleParticipantStatus(
                                                            p.id,
                                                            p.status
                                                        )
                                                    )
                                ...
                                Box(
                                    modifier = Modifier.width(1.dp).height(100.dp)
                                ...
                                Column(modifier = Modifier.weight(1f).padding(start = 16.dp)) {
                                    ...
                                    Spacer(modifier = Modifier.height(8.dp))
                                    match.participants.filter { it.teamSide == "GUEST" }
                                        .forEach { p ->
                                            ParticipantRow(
                                                name = p.playerName,
                                                status = p.status,
                        ...
                        if (state.isUserInvolved) {
                            item {
                                ...
                                if (!state.hasApplied) {
                                    ...
                                    val statusMsg =
                                        if (state.myStatus == "SELECTED") "Be vagy válogatva a meccskeretbe!" else "Jelentkezésed rögzítve, várj a kapitány döntésére."
                                    val statusColor =
                                        if (state.myStatus == "SELECTED") Color(0xFF4CAF50) else MaterialTheme.colorScheme.primary
                                    ...
                                    Column(horizontalAlignment = Alignment.CenterHorizontally) {
                                        ...
                                        // Visszavonás csak akkor, ha MÁR JELENTKEZETT, de a kapitány még nem véglegesítette (itt feltételezzük, hogy "SELECTED"-ből is ki lehet lépni a véglegesítés előtt)
                                        Spacer(modifier = Modifier.height(8.dp))
                                        TextButton(onClick = { viewModel.onEvent(MatchDetailsEvent.OnWithdrawApplication) }) {
                                            Text(
                                ...
                                // B) KAPITÁNYI NÉZET: Véglegesítés (ha van elég ember)
                                val isCaptainOfCurrentMatch =
                                    state.isHomeCaptain || state.isGuestCaptain
                                val selectedCount =
                                    if (state.isHomeCaptain) state.homeSelectedCount else state.guestSelectedCount
...
// Segéd komponens a játékosok megjelenítéséhez
@Composable
fun ParticipantRow(
    name: String,
    status: String,
